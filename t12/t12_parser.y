...
extern  char    *t12yytext;

extern  void    t12yyerror(const char *s);
%}
...
%token  INSTR_CMP_EQ_BOOL
%token  INSTR_CMP_NE_BOOL
%token  INSTR_CMP_GT_BOOL
%token  INSTR_CMP_GE_BOOL
%token  INSTR_CMP_LT_BOOL
%token  INSTR_CMP_LE_BOOL
...
instr   :   TOKEN_ID COLON
    ...
    |   INSTR_BOOL_CONST reg COMMA BOOL_CONST
        {
            Instr *instr = record_instr();
    ...
    |   INSTR_CMP_EQ_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_EQ_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
    |   INSTR_CMP_NE_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_NE_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
    |   INSTR_CMP_GT_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_GT_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
    |   INSTR_CMP_GE_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_GE_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
    |   INSTR_CMP_LT_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_LT_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
    |   INSTR_CMP_LE_BOOL reg COMMA reg COMMA reg
        {
            Instr *instr = record_instr();
            instr->opcode = OP_CMP_LE_BOOL;
            instr->rd = $2;
            instr->rs1 = $4;
            instr->rs2 = $6;
        }
...
reg :   TOKEN_REG
            ...
            if ($1 < NUMBER_REGS) {
                ...
                printf("%s, %d: ", t12yyfile, t12yylinenum);
...
void t12yyerror(const char *s)
{
    ...
    else if (isprint(t12yytext[0]))
        sprintf(buf, "%s at symbol `%s'", s, t12yytext);
    else
        sprintf(buf, "%s at \\%o", s, t12yytext[0]);
    
    printf("%s, %d: %s\n", t12yyfile, t12yylinenum, buf);
}
